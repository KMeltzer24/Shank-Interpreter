...
import com.shank.nodes.*;
import com.shank.builtin.*;
...
public class Interpreter {
	 ...
	 */
	private static final HashMap<String, CallableNode> functMap = new HashMap<>();
	...
	public static int resolveInt(Node node, HashMap<String, InterpreterDataType> varHash) {
		if (node instanceof IntegerNode integerNode) {					// Returns the value of the node if it is a integerNode
			return integerNode.getInteger();
		} else if (node instanceof VariableReferenceNode variableReferenceNode) {	
			return Integer.parseInt(varHash.get(variableReferenceNode.toString()).toString());
		} else if (node instanceof MathOpNode mathOpNode) {
			return switch (mathOpNode.getState()) {		//Iterates through the ast to resolve the result
				case PLUS -> resolveInt(mathOpNode.getLeft(), varHash) + resolveInt(mathOpNode.getRight(), varHash);
				case MINUS -> resolveInt(mathOpNode.getLeft(), varHash) - resolveInt(mathOpNode.getRight(), varHash);
				case TIMES -> resolveInt(mathOpNode.getLeft(), varHash) * resolveInt(mathOpNode.getRight(), varHash);
				default -> resolveInt(mathOpNode.getLeft(), varHash) / resolveInt(mathOpNode.getRight(), varHash);
			};
		}
		return -1;
	}
	...
	public static Float resolveFloat(Node node, HashMap<String, InterpreterDataType> varHash) {
		if (node instanceof FloatNode floatNode) {					// Returns the value of the node if it is a FloatNode
			return floatNode.getFloat();
		} else if (node instanceof VariableReferenceNode variableReferenceNode) {	
			return Float.valueOf(varHash.get(variableReferenceNode.toString()).toString());
		} else if (node instanceof MathOpNode mathOpNode) {
			return switch (mathOpNode.getState()) {
				case PLUS -> resolveFloat(mathOpNode.getLeft(), varHash) + resolveFloat(mathOpNode.getRight(), varHash);
				case MINUS -> resolveFloat(mathOpNode.getLeft(), varHash) - resolveFloat(mathOpNode.getRight(), varHash);
				case TIMES -> resolveFloat(mathOpNode.getLeft(), varHash) * resolveFloat(mathOpNode.getRight(), varHash);
				default -> resolveFloat(mathOpNode.getLeft(), varHash) / resolveFloat(mathOpNode.getRight(), varHash);
			}; //Iterates through the ast to resolve the result
		}
		return (float)-1;
	}
	...
	public static String resolveString(Node node, HashMap<String, InterpreterDataType> varHash) throws Exception {
		if (node instanceof StringNode) {				// Returns the value of the node if it is a StringNode
			...
			return varHash.get(variableReferenceNode.toString()).toString();
		} else if (node instanceof MathOpNode mathOpNode) {
			if (mathOpNode.getState().equals(MathOpNode.operator.PLUS)) {	//Iterates through the ast to resolve the result
	...
	public static char resolveCharacter(Node node, HashMap<String, InterpreterDataType> varHash) throws Exception {
		if (node instanceof CharNode) {			// Returns the value of the node if it is a CharNode
			...
			return varHash.get(variableReferenceNode.toString()).toString().charAt(0);
		} else {
	...
	public static boolean resolveBoolean(Node node, HashMap<String, InterpreterDataType> varHash) throws Exception {
		if (node instanceof VariableReferenceNode variableReferenceNode) {	
			return Boolean.parseBoolean(varHash.get(variableReferenceNode.toString()).toString());
		} else if (node instanceof BooleanExpressionNode booleanExpressionNode) {	
			if (booleanExpressionNode.getRight() == null) {	
				return Boolean.parseBoolean(booleanExpressionNode.getLeft().toString());
			} else if (booleanExpressionNode.getLeft() == null) {	
				return Boolean.parseBoolean(varHash.get(booleanExpressionNode.getRight().toString()).toString());
			} else {
				return EvaluateBooleanExpression(booleanExpressionNode, varHash);
			}
		}
		return false;
	}
	...
	public static boolean EvaluateBooleanExpression(BooleanExpressionNode node, HashMap<String, InterpreterDataType> varHash) throws Exception {
		...
		if (node.getLeft() instanceof VariableReferenceNode) {		// Gets left node from the expression
			InterpreterDataType leftNodeType = varHash.get(node.getLeft().toString());
			if (leftNodeType instanceof IntDataType) {
				left = new IntegerNode(Integer.parseInt(leftNodeType.toString()));
			} else if (leftNodeType instanceof FloatDataType) {
				left = new FloatNode(Float.valueOf(leftNodeType.toString()));
			} else {
		...
		if (node.getRight() instanceof VariableReferenceNode) {		// Gets right node from the expression
			InterpreterDataType rightNodeType = varHash.get(node.getRight().toString());
			if (rightNodeType instanceof IntDataType) {
				right = new IntegerNode(Integer.parseInt(rightNodeType.toString()));
			} else if (rightNodeType instanceof BooleanDataType) {
				return Boolean.parseBoolean(varHash.get(node.getRight().toString()).toString());
			} else if (rightNodeType instanceof FloatDataType) {
				right = new FloatNode(Float.valueOf(rightNodeType.toString()));
			} else {
				throw new Exception("Can only evaluate a boolean expression with numbers");
			}
		} else {
		...
		String typeL = left.getClass().getSimpleName();			// Sets as string to the type of the left node
		String typeR = right.getClass().getSimpleName();			// Sets as string to the type of the right node
		if (left instanceof MathOpNode mathOpNode) {
			typeL = mathOpNode.getLeft().getClass().getSimpleName();
		} else if (right instanceof MathOpNode mathOpNode) {
			typeR = mathOpNode.getLeft().getClass().getSimpleName();
		}
		...
		switch(node.getCondition()) {
			...
			case "NOTEQUAL" -> {
				if (typeL.equals("IntegerNode")) {
					...
					return !(resolveFloat(left, varHash)).equals(resolveFloat(right, varHash));
				}
			}
	...
	public static void InterpretFunction(FunctionNode function, ArrayList<InterpreterDataType> dataTypes) throws Exception {
		...
		HashMap<String, InterpreterDataType> varHash = new HashMap<>();
		...
		for(int i = 0; i < localVars.size(); i++) {	
			if (localVars.get(i).isConstant()) {
				switch (localVars.get(i).getState()) {
					case INTEGER -> varHash.put(localVars.get(i).getName(), new IntDataType(Integer.parseInt(localVars.get(i).getInitialValue().toString())));
					case REAL -> varHash.put(localVars.get(i).getName(), new FloatDataType(Float.valueOf(localVars.get(i).getInitialValue().toString())));
					case STRING -> varHash.put(localVars.get(i).getName(), new StringDataType(localVars.get(i).getInitialValue().toString()));
					case CHAR -> varHash.put(localVars.get(i).getName(), new CharDataType(localVars.get(i).getInitialValue().toString().charAt(0)));
					default -> varHash.put(localVars.get(i).getName(), new BooleanDataType(Boolean.parseBoolean(localVars.get(i).getInitialValue().toString())));
				}
			} else {
				switch (localVars.get(i).getState()) {
					case INTEGER -> varHash.put(localVars.get(i).getName(), new IntDataType());
					case REAL -> varHash.put(localVars.get(i).getName(), new FloatDataType());
					case STRING -> varHash.put(localVars.get(i).getName(), new StringDataType());
					case CHAR -> varHash.put(localVars.get(i).getName(), new CharDataType());
					default -> varHash.put(localVars.get(i).getName(), new BooleanDataType());
				}
			}	
		}
	...
	public static void InterpretFunctionCall(int i, ArrayList<StatementNode> statements, HashMap<String, InterpreterDataType> varHash) throws Exception {
		String functionName = ((FunctionCallNode)statements.get(i)).getFunctionName();
		ArrayList<ParameterNode> paramList = ((FunctionCallNode)statements.get(i)).getParameters();
		ArrayList<InterpreterDataType> dataTypeList = new ArrayList<>(); 
		...
		for (int x = 0; x < paramList.size(); x++) {
			if (paramList.get(x).getNode() instanceof IntegerNode) {
				...
				dataTypeList.add(new FloatDataType(Float.valueOf(paramList.get(x).getNode().toString())));
		...
		switch(functionName) {
			case "read" -> {
				ArrayList<VariableNode> readVars = new ArrayList<>();
				String name = "a";
	...
	public static void InterpretAssignment(int i, ArrayList<StatementNode> statements, HashMap<String, InterpreterDataType> varHash) throws Exception {
		...
		if (varHash.get(target) instanceof IntDataType) {
			varHash.put(target, new IntDataType(resolveInt(expression, varHash))); 
